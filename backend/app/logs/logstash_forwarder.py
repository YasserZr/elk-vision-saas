...
class LogstashForwarder:
            ...
            try:
                ...
                for log_entry in log_entries:
                    if self._send_single_entry(log_entry):
                        ...
                        logger.warning(
                            f"Failed to send log entry: {log_entry.get('message', '')[:100]}"
                        )
                ...
                if success_count == len(log_entries):
                    logger.info(
                        f"Successfully sent {success_count} log entries to Logstash"
                    )
                    return True
                else:
                    logger.warning(
                ...
                logger.error(
                    f"Unexpected error sending logs to Logstash: {e}", exc_info=True
                )
                self._stats["last_error"] = str(e)
    ...
    def send_batch(
        self, log_entries: List[Dict[str, Any]], batch_size: int = 100
    ) -> Dict[str, int]:
        """
        ...
        return {"sent": total_sent, "failed": total_failed, "total": len(log_entries)}
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_success": (
                time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(self._stats["last_success"])
                )
                if self._stats["last_success"]
                else None
            ),
        }
