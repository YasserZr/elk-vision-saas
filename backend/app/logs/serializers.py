...
class LogFileUploadSerializer(serializers.Serializer):
    ...
    source = serializers.CharField(max_length=255, required=False, default='manual_upload')
    environment = serializers.ChoiceField(
        choices=['production', 'staging', 'development', 'testing'],
        required=False,
        default='production'
    )
    service_name = serializers.CharField(max_length=255, required=False, default='unknown')
    ...
    def validate_file(self, file):
        ...
        if content_type not in ALLOWED_CONTENT_TYPES:
            logger.warning(f"Unusual content type: {content_type} for file: {file.name}")
        ...
        for i, line in enumerate(lines[:10]):  # Check first 10 lines
            if len(line) > 10000:  # 10KB per line
                raise ValidationError(
                    f"Line {i+1} is too long (max 10KB per line)"
                )
